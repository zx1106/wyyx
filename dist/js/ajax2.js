...
function ajax(options) {
  var default1 = {
    ...
    //请求成功的回调函数
    error1: function error1() {} //请求失败的回调函数
  ...
  if (default1.data) {
    ...
    if (default1.type.toLowerCase() == 'get') {
      //配置请求信息
      xhr.open(default1.type, default1.url + '?' + default1.data, default1.async); //发送请求

      xhr.send();
    } else if (default1.type.toLowerCase() == 'post') {
      //配置请求信息
      xhr.open(default1.type, default1.url, default1.async); //设置请求头
    ...
    //配置请求信息
    xhr.open(default1.type, default1.url, default1.async); //发送请求
